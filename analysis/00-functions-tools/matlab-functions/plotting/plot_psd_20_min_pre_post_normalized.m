...
function plot_psd_20_min_pre_post_normalized(psd_1, psd_2, main_title, dir_rec, hpc_reg, save_flag)
...
for epoch = epochs
    ...
    for minute = 1:size(psd_1.B,1) % numb of minute
        ...
        plot(f,log10(psd_1.(epoch)(minute,:)/sum(psd_1.(epoch)(minute,:))),'LineWidth', 2, 'Color', baseColor(i,:)); hold on % first day
        plot(f,log10(psd_2.(epoch)(minute,:)/sum(psd_2.(epoch)(minute,:))),'LineWidth', 2, 'Color', darkColor); hold on % after a few days (boost)
    
        
        % accumulate values at each minute to compute average results
        psd_1_tot = [psd_1_tot; log10(psd_1.(epoch)(minute,:)/sum(psd_1.(epoch)(minute,:)))];
        psd_2_tot = [psd_2_tot; log10(psd_2.(epoch)(minute,:)/sum(psd_2.(epoch)(minute,:)))];   
    ...
    ylabel('Norm log(PSD)')
    legend('PRE','POST')
    title(sprintf('Avg across minutes NORMALIZED - %s', ep_names(i)),'FontSize',10);
    ...
    if save_flag
        ...
        saveas(fig, strcat(dir_out,sprintf('\\20_min_psd_pre_post_epoch_%s_reg_%s_normalized.png',epoch, hpc_reg) ))
        saveas(fig_avg, strcat(dir_out,sprintf('\\20_min_psd_AVG_pre_post_epoch_%s_reg_%s_normalized.png',epoch, hpc_reg) ))
        
        saveas(fig, strcat(dir_out,sprintf('\\20_min_psd_pre_post_epoch_%s_reg_%s_normalized.pdf',epoch, hpc_reg) ))
        saveas(fig_avg, strcat(dir_out,sprintf('\\20_min_psd_AVG_pre_post_epoch_%s_reg_%s_normalized.pdf',epoch, hpc_reg) ))
    end
