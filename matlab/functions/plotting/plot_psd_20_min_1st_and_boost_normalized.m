...
function plot_psd_20_min_1st_and_boost_normalized(psd_1, psd_2, main_title, dir_rec, hpc_reg, save_flag, method)
...
for epoch = epochs
    ...
    for min = 1:size(psd_1.B,1) % numb of minute
        subplot(5,4,min)
        plot(f,log10(psd_1.(epoch)(min,:)/sum(psd_1.(epoch)(min,:))),'LineWidth', 2, 'Color', baseColor(i,:)); hold on % first day
        plot(f,log10(psd_2.(epoch)(min,:)/sum(psd_2.(epoch)(min,:))),'LineWidth', 2, 'Color', darkColor); hold on % after a few days (boost)
    ...
    if save_flag
        ...
        saveas(fig, strcat(dir_out,sprintf('\\20_min_psd_1st_and_boost_epoch_%s_reg_%s_normalized.png',epoch, hpc_reg) ))
    end
