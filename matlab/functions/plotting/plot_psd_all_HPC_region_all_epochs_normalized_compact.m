...
ha = tight_subplot(1,4,[.0 .02],[.15 .2],[.06 .07]); % [gap_h gap_w] , margin [lower upper] [left right]

for ii = 1:4
    % BASELINE
    axes(ha(ii));
      
    plot(f,log10(psd.so.(epoch(ii))(minute,:)/sum(psd.so.(epoch(ii))(minute,:))),'LineWidth', 2, 'Color', '#ef476f'); hold on
    plot(f,log10(psd.sp.(epoch(ii))(minute,:)/sum(psd.sp.(epoch(ii))(minute,:))),'LineWidth', 2, 'Color', '#f78c6b'); hold on
    plot(f,log10(psd.rad.(epoch(ii))(minute,:)/sum(psd.rad.(epoch(ii))(minute,:))),'LineWidth', 2, 'Color', '#ffd166'); hold on
    plot(f,log10(psd.lm.(epoch(ii))(minute,:)/sum(psd.lm.(epoch(ii))(minute,:))),'LineWidth', 2, 'Color', '#06d6a0'); hold on
    plot(f,log10(psd.dup.(epoch(ii))(minute,:)/sum(psd.dup.(epoch(ii))(minute,:))),'LineWidth', 2, 'Color', '#118ab2'); hold on
    plot(f,log10(psd.dd.(epoch(ii))(minute,:)/sum(psd.dd.(epoch(ii))(minute,:))),'LineWidth', 2, 'Color', '#073b4c'); 
...
if save_flag
    ...
    saveas(fig, strcat(dir_out,sprintf('\\psd_normalized_all_epochs_all_regions_min_%d_%s.pdf', minute, method) ))
end
