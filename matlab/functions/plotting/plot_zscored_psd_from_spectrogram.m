...
% BASELINE 
epoch = 'B';
psd_CA1 = mean(zscore(log10(spec_rec.(epoch).CA1(:,:,min)),1,2),1);
psd_ripple = mean(zscore(log10(spec_rec.(epoch).ripple(:,:,min)),1,2),1);
psd_rad = mean(zscore(log10(spec_rec.(epoch).rad(:,:,min)),1,2),1);
psd_lm = mean(zscore(log10(spec_rec.(epoch).lm(:,:,min)),1,2),1);
psd_dup = mean(zscore(log10(spec_rec.(epoch).dup(:,:,min)),1,2),1);
psd_dd = mean(zscore(log10(spec_rec.(epoch).dd(:,:,min)),1,2),1);
...
if save
    ...
    saveas(fig,strcat(dir_out,sprintf('\\psd_from_zscored_Spec_all_epochs_all_areas_min_%d.png',min_lab) ) )
end
