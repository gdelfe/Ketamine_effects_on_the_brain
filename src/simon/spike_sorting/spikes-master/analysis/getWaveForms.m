...
%{
gwfparams.dataDir = drsDAT{baI};    % KiloSort/Phy output folder
gwfparams.fileName = fileNamesDAT{baI};         % .dat file containing the raw 
gwfparams.dataType = 'int16';            % Data type of .dat file (this should be BP filtered)
gwfparams.nCh = NchannelsDAT+8;                      % Number of channels that were streamed to disk in .dat file
gwfparams.wfWin = [-40 41];              % Number of samples before and after spiketime to include in waveform
gwfparams.nWf = 2000;                    % Number of waveforms per unit to pull out
gwfparams.spikeTimes =    tsSelected; % Vector of cluster spike times (in samples) same length as .spikeClusters
gwfparams.spikeClusters = cluSelected; % Vector of cluster IDs (Phy nomenclature)   same length as .spikeTimes
%}
